...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const BANK_SAVINGS_RATE = 0.05;   // 예금 이자 (연 5%)
    const BANK_LOAN_RATE = 0.20;      // 대출 이자 (연 20%)
    ...
    function handleBankrupt() {
        ...
        playerRef.transaction((currentPlayerData) => {
            ...
            // (자산 리셋)
            currentPlayerData.stocks = {}; // 주식 청산
            currentPlayerData.bank.savings = 0; // 예금 청산
            currentPlayerData.bank.savingsTimestamp = null;
            
            // (신규 상태 설정)
            currentPlayerData.cash = 3000000; // 현금 300만
            currentPlayerData.bank.loan = 5000000; // 대출 500만
            
            // (쿨타임 타이머 시작)
            currentPlayerData.bank.loanTimestamp = firebase.database.ServerValue.TIMESTAMP;
            currentPlayerData.bank.loanRepayTimestamp = null; // 대출 쿨타임은 리셋
            currentPlayerData.bank.bankruptTimestamp = firebase.database.ServerValue.TIMESTAMP; // 파산 쿨타임 시작
    ...
    // (상환: 현금 -> 대출)
    function handleBankRepay() {
        playerRef.transaction((data) => {
            if (!data) return;
            
            const { currentLoan } = calculateInterest(data.bank);
            if (currentLoan <= 0) {
                showAlert("상환할 대출금이 없습니다.");
                return;
            }
            if (data.cash < currentLoan) {
                showAlert("대출금 전액을 상환할 현금이 부족합니다.");
                return;
            }
            
            data.cash -= currentLoan;
            data.bank.loan = 0; // 전액 상환
            data.bank.loanTimestamp = null;
    ...
    function handleStartTimeAttack() {
        ...
        // ▲▲▲ [버그 수정] ▲▲▲
            ...
            playerRef.transaction((currentPlayerData) => {
                ...
                currentPlayerData.snapshot = snapshotData; // (백업 저장)
                ...
                currentPlayerData.timeAttack.startTime = firebase.database.ServerValue.TIMESTAMP;
            ...
            .then((result) => {
                if (result.committed) {
                    // (트랜잭션 성공 -> setupPlayerListener가 UI를 자동 갱신할 것임)
                    showAlert("✅ 타임 어택 시작!", "30분간 초기 자본으로 최대 수익에 도전하세요. 기존 데이터는 안전하게 보관됩니다.");
                    // (타이머 시작)
                    startTimeAttackTimer(TIME_ATTACK_DURATION);
                    els.timeAttackBtn.disabled = true;
                    els.timeAttackBtn.textContent = "타임 어택 진행 중...";
                    els.timeAttackTimerDisplay.classList.remove('hidden');
                    els.timeAttackCancelBtn.classList.remove('hidden');
                } else {
                    // (트랜잭션이 중단된 경우 - 이미 알림 뜸)
                }
            })
            .catch((error) => {
    ...
    function startTimeAttackTimer(duration) {
        ...
        updateTimerDisplay(); // 즉시 05:00 표시
    ...
    function handleCancelTimeAttack() {
        if (!state.timeAttack || !state.timeAttack.isInTimeAttack) {
            return showAlert("현재 타임 어택이 진행 중이지 않습니다.");
        }
        
        showConfirm("타임 어택을 중지(취소)하시겠습니까?<br><br>현재까지의 진행 상황은 저장되지 않으며, 원래 데이터로 즉시 복구됩니다.", () => {
            ...
            // (2) 트랜잭션으로 스냅샷 복구 (handleEndTimeAttack과 유사하나 점수 계산 없음)
            playerRef.transaction((currentPlayerData) => {
                if (!currentPlayerData) return;
                ...
                // [치명적 오류 방어]
                if (!currentPlayerData.snapshot) {
                    console.error("심각한 오류: 타임 어택 취소 시 스냅샷 데이터가 없습니다!");
                    currentPlayerData.timeAttack.isInTimeAttack = false;
                    return currentPlayerData;
                }
                ...
                // 2. 현재 데이터를 -> 스냅샷 데이터로 덮어쓰기 (복원)
                currentPlayerData.cash = restoredData.cash;
                currentPlayerData.stocks = restoredData.stocks;
                currentPlayerData.bank = restoredData.bank;
                
                // 3. 스냅샷 및 타임 어택 상태 초기화
            ...
            .then((result) => {
                if (result.committed) {
                    showAlert("타임 어택이 취소되었습니다.", "기존 게임 데이터를 복구했습니다.");
            ...
            .catch((error) => {
                console.error("타임 어택 취소 트랜잭션 오류:", error);
                showAlert("타임 어택 취소/복원에 실패했습니다. (DB 오류)");
            });
        });
    }
    ...
    function renderRankings() {
        ...
        rankingsToDisplay.slice(0, 20).forEach((entry, index) => {
    ...
    function updateBankTimerUI() {
        ...
        if (els.bankSavingsAmount) {
            ...
            const nextInterest = savings * 0.03;
            els.bankNextInterest.textContent = `+ ${formatCurrency(nextInterest)}`;
            ...
            if (savings <= 0 || !savingsTime) {
                ...
                if (remainingMs <= 0) {
                    els.bankNextInterestTimer.textContent = "이자 지급 대기중...";
                } else {
                    const remainingSeconds = Math.floor(remainingMs / 1000);
                    const minutes = Math.floor(remainingSeconds / 60);
                    const seconds = remainingSeconds % 60;
    ...
    function calculateInterest(bankData) {
        const now = Date.now();
        const secondsPerYear = 365 * 24 * 60 * 60; // 1년 (초)
        
        // 예금 이자
        let currentSavings = Number(bankData.savings) || 0;
        if (currentSavings > 0 && bankData.savingsTimestamp) {
            const secondsElapsed = (now - bankData.savingsTimestamp) / 1000;
            const yearsElapsed = secondsElapsed / secondsPerYear;
            currentSavings = currentSavings * Math.pow(1 + BANK_SAVINGS_RATE, yearsElapsed);
        }
        
        // 대출 이자
        let currentLoan = Number(bankData.loan) || 0;
        if (currentLoan > 0 && bankData.loanTimestamp) {
            const secondsElapsed = (now - bankData.loanTimestamp) / 1000;
            const yearsElapsed = secondsElapsed / secondsPerYear;
            currentLoan = currentLoan * Math.pow(1 + BANK_LOAN_RATE, yearsElapsed);
        }
        
        return { currentSavings, currentLoan };
    }
    ...
    function createInitialMarketState() {
        ...
        const marketStocks = {
            ...
            'MSFT': createStock('Microsoft (마이크로소프트)', 35000, 0.018, 0.02, 0.54), // 0.52 -> 0.54
            'GOOGL': createStock('Alphabet (구글)', 14000, 0.02, 0.03, 0.53), // 0.51 -> 0.53
            'AMZN': createStock('Amazon (아마존)', 15000, 0.025, 0.04, 0.53), // 0.51 -> 0.53
            'NVDA': createStock('NVIDIA (엔비디아)', 45000, 0.04, 0.08, 0.54), // 0.53 -> 0.54
            'META': createStock('Meta Platforms (메타)', 30000, 0.035, 0.06, 0.52), // 0.50 -> 0.52
            'TSLA': createStock('Tesla (테슬라)', 25000, 0.05, 0.1, 0.52), // 0.50 -> 0.52
            'BRK-B': createStock('Berkshire Hathaway B (버크셔 해서웨이 B)', 36000, 0.01, 0.01, 0.54), // 0.52 -> 0.54
            'V': createStock('Visa (비자)', 24000, 0.015, 0.02, 0.54), // 0.52 -> 0.54
            ...
            'NFLX': createStock('Netflix (넷플릭스)', 40000, 0.035, 0.06, 0.52), // 0.50 -> 0.52
            ...
            'DIS': createStock('Disney (디즈니)', 9000, 0.025, 0.04, 0.52), // 0.50 -> 0.52
            ...
            'COST': createStock('Costco (코스트코)', 50000, 0.015, 0.02, 0.54), // 0.52 -> 0.54
            'PEP': createStock('PepsiCo (펩시코)', 17000, 0.01, 0.01, 0.53), // 0.51 -> 0.53
            'HD': createStock('Home Depot (홈디포)', 30000, 0.017, 0.02, 0.53), // 0.51 -> 0.53
            'SEC': createStock('삼성전자 (Samsung Elec.)', 75000, 0.018, 0.03, 0.54), // 0.52 -> 0.54
            'SKH': createStock('SK하이닉스 (SK Hynix)', 20000, 0.025, 0.05, 0.53), // 0.51 -> 0.53
            'LGES': createStock('LG에너지솔루션 (LG Energy Solution)', 35000, 0.03, 0.06, 0.52), // 0.50 -> 0.52
            'HYMT': createStock('현대자동차 (Hyundai Motor)', 25000, 0.02, 0.04, 0.53), // 0.51 -> 0.53
            'NAVER': createStock('네이버 (NAVER)', 17000, 0.035, 0.07, 0.52), // 0.50 -> 0.52

            // --- 실물자산 (ASSETS) ---
            'GOLD': createStock('금 (Gold)', 20000, 0.01, 0.02, 0.53), // 0.51 -> 0.53
            'SLVR': createStock('은 (Silver)', 2500, 0.02, 0.04, 0.52), // 0.50 -> 0.52
            'OIL': createStock('WTI 원유 (Crude Oil)', 8000, 0.03, 0.08, 0.52), // 0.50 -> 0.52
            'NGAS': createStock('천연가스 (Natural Gas)', 3000, 0.09, 0.15, 0.52), // 0.50 -> 0.52
            'COPR': createStock('구리 (Copper)', 8500, 0.025, 0.05, 0.53), // 0.51 -> 0.53
            'WHEAT': createStock('밀 (Wheat)', 6000, 0.05, 0.10, 0.52), // 0.50 -> 0.52

            // --- 채권 (BONDS) ---
            'BOND_L': createStock('미국 장기채 (US Long Bond)', 10000, 0.005, 0.01, 0.525), // 0.505 -> 0.525
            'BOND_S': createStock('미국 단기채 (US Short Bond)', 5000, 0.002, 0.00, 0.53), // 0.51 -> 0.53
            'CORP_B': createStock('미국 회사채 (Corp. Bond)', 9000, 0.008, 0.01, 0.53), // 0.51 -> 0.53
            'HY_B': createStock('하이일드 채권 (High-Yield)', 7000, 0.015, 0.03, 0.52), // 0.50 -> 0.52

            // --- 코인 (COINS) ---
            'BTC': createStock('비트코인 (Bitcoin)', 60000, 0.08, 0.15, 0.52), // 0.50 -> 0.52
            'ETH': createStock('이더리움 (Ethereum)', 3000, 0.10, 0.18, 0.52), // 0.50 -> 0.52
            'DOGE': createStock('도지코인 (Dogecoin)', 100, 0.20, 0.30, 0.52), // 0.50 -> 0.52
            'SOL': createStock('솔라나 (Solana)', 1500, 0.15, 0.25, 0.52), // 0.50 -> 0.52
            
            // --- 기타 (MISC) ---
            'DEV_MOOD': createStock('개발자 무드 코인 (Dev Mood Coin)', 1000, 0.0, 0.0, 0), // 0.50 -> 0.52
            'SONG': createStock('송송그룹 (Song)', 100000, 0.12, 0.05, 0.532),
            'COOKIE': createStock('쿠키컴퍼니 (CookieCo)', 7500, 0.25, 0.3, 0.532)
        };
        return marketStocks;
    }


    // === 16. Game Loop (Price Update) ===
    
    /**
     * 1초마다 주가 업데이트 루프 시작 (관리자만 실행)
     */
    function startPriceUpdateLoop() {
        // (이 로직은 서버(Cloud Function)로 옮기는 것이 이상적이지만,
        //  현재는 클라이언트(브라우저)에서 실행합니다.)
        
        // (참고: 여러 클라이언트가 동시에 루프를 실행하는 것을 방지하려면
        //  DB에 'isLoopRunning' 플래그를 두어야 하지만, 여기서는 생략)
        
        console.log("주가 업데이트 루프 시작 (1초마다)");
    ...
    function updateStockPrices() {
        ...
        marketRef.child('stocks').transaction((currentMarketData) => {
            ...
            Object.keys(currentMarketData).forEach(ticker => {
                ...
                let riseProbability = stock.baseRiseProbability;
